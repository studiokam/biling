...
$temat= "=?UTF-8?B?".base64_encode("Przypomnienie o abonamencie za serwer. Koniec abonamentu za 15 dni.")."?=";
$tematAdmin= "=?UTF-8?B?".base64_encode("Przypomnienie o abonamencie za serwer. Koniec abonamentu za 15 dni. (kopia)")."?=";

// zmienne tematów do zapytań (logi) aby nie wstawialo z tymi znakami specjalnymi
$tematLogi = "Przypomnienie o abonamencie za serwer. Koniec abonamentu za 15 dni.";
$tematAdminLogi = "Przypomnienie o abonamencie za serwer. Koniec abonamentu za 15 dni. (kopia)";
...
$tresc_zapytania = "SELECT * FROM klienci WHERE do_kiedy_data > $dataUnixPlus14 AND do_kiedy_data < $dataUnixPlus15";
			...
			return '<!DOCTYPE HTML>
						...
						.center {
						    ...
						    <div class="center">
						      <p>Witamy	</p>
						      <p>&nbsp;</p>
						      <p>Przypominamy, że Pana/Pani usługa hostingowa przypisana do konta <b>'.$user.'</b> jest opłacona do dnia: '.date("<b>Y.m.d</b>", $data).' (pozostało 15 dni). <br>
						        <br>
						        Do końca okresu pozostało 15 dni. Wyślemy jeszcze jedno przypomnienie dnia ostatniego a po tym usługa zostanei wyłączona.Po wyłączeniu usługi możliwe będzie jeszcze jej przywrócenie przez okres 14 dni. 
						        <br>
...
echo '<br><br>
	...
	foreach ($dbConnect->query("SELECT user,email,usluga,do_kiedy_data,id  FROM klienci WHERE do_kiedy_data > $dataUnixPlus14 AND do_kiedy_data < $dataUnixPlus15") as $k) 
	{
		$szukajN1 = 'N1';
		$szukajN2 = 'N2';
		$szukajN3 = 'N3';
		...
		if (in_array($szukajN1, $k)) 
		{
			...
			if (mail) 
			{
				$sql = mysql_query("INSERT INTO logi SET tresc='Wysłano wiadomość do usera $user. Temat: $tematLogi. Dodał:  ', zalogowany='automat', id_usera='$idUsera'");
			}
			echo '
				<tr class="active">
...
else 
{
	...
	echo '
		<table class="table table-hover table-bordered">
	 		<tr class="active">
	 			<td>Dnia <b>('.$dataDodania.')</b> nie było userów spełniających warunek: 15 dni do końca abonamentu.</td>
	 		</tr>
	 		<tr>
				<td>
					<a class="btn btn-success " type="submit" href="ustawienia.php">Przejdź do ustawień</a>
	...
	';
	mysql_query("INSERT INTO logi SET tresc='Dnia <b>($data)</b> nie było userów spełniających warunek: 15 dni do końca abonamentu. Dodał:  ', zalogowany='automat'");
}

mysql_query("INSERT INTO logi SET tresc='SPRAWDZENIEco15dni', zalogowany=' - automat'");
