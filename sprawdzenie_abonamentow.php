			...
			return '<!DOCTYPE HTML>
						...
						.center {
						    ...
						    <div class="center">
						      <p>Witamy	</p>
						      <p>&nbsp;</p>
						      <p>Przypominamy, że Pana/Pani usługa hostingowa przypisana do konta <b>'.$user.'</b> jest opłacona do dnia: '.date("<b>Y.m.d</b>", $data).' (Ostatni dzień). <br>
...
echo '<br><br>
	...
	foreach ($dbConnect->query("SELECT user,email,usluga,do_kiedy_data,id  FROM klienci WHERE do_kiedy_data > $dataUnix AND do_kiedy_data < $dataUnixPlusDoba") as $k) 
	{
		$szukajN1 = 'N1';
		$szukajN2 = 'N2';
		$szukajN3 = 'N3';
		...
		if (in_array($szukajN1, $k)) 
		{
			...
			$usluga = $k[2];
			$data = $k[3];
			$dataNormalnyFormat = date("Y.m.d H:i:s", $k[3]);
			$idUsera = $k[4];
...
mysql_query("INSERT INTO logi SET tresc='SPRAWDZENIEco24', zalogowany=' - automat'");
